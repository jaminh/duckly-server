...
def main(global_config, **settings):
    ...
    config = Configurator(settings = settings, session_factory=session_factory_from_settings(settings))
    ...
    Compiler.filters['css'] = lambda x,y: '<style type="text/css">{0}</style>'.format(x)
    Compiler.filters['javascript'] = lambda x,y: '<script type="text/javascript">{0}</script>'.format(x)
